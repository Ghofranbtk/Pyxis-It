...
def lire_points(fichier):
    """ Lit un fichier CSV contenant les colonnes contentType, Date, value et retourne un tableau NumPy """
    df = pd.read_csv(fichier, usecols=["Date", "value"], parse_dates=["Date"])  # Lire les colonnes nécessaires
    df["Date"] = df["Date"].astype("int64") // 10**9  # Convertir Date en timestamp (secondes depuis l'époque)
...
# Nom des fichiers
fichier_points = "jour.csv"  # Fichier CSV en entrée
fichier_reduit = "points_reduits.csv"  # Fichier CSV en sortie

# Lecture des points
points = lire_points(fichier_points)

# Application de la réduction avec un seuil donné
epsilon = 1e1  # Seuil pour la différence en Y
seuil_pente = 1e1  # Seuil pour la différence de pente
...
pd.DataFrame(reduced_points, columns=['Timestamp', 'Value']).to_csv(fichier_reduit, index=False)
